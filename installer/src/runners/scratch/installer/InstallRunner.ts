...
// For each step we need a few things
// 1. The hosts to run the commands on (and whether it's an agent or server install)
// 2. Configuration items

export const InstallRunnerMachine = createMachine({
  id: "installRunner",
  initial: "idle",
  context: {
    commands: [],
  ...
  states: {
    ...
    // We first install the K3s server
    installingK3sServer: {},
    // After the server has been installed
    // we install K3s agents.
    // This is done in parallel
    installingK3sAgents: {},
    // Label nodes for whether
    // the should be worker, storage, etc.
    // Also add labels for topology
    // such as region, zone, etc.
    labelingNodes: {},
    // Configure storage
    // We setup all necessary storage
    // on nodes with LVM
    // This is also done in parallel
    configuringStorage: {},
    // Install packages
    // We run these one after the other
 ...
 *
 * The InstallRunner will take a installation
 * configuration and run it one the hosts
 * specified in the configuration.
 */
export class InstallRunner {
  private actor: ReturnType<typeof createActor<typeof InstallRunnerMachine>>;
  private userContext: Record<string, unknown> = {};

  constructor(subscribeHooks?: Observer<any>[]) {
    this.actor = createActor(InstallRunnerMachine, {
      context: {
